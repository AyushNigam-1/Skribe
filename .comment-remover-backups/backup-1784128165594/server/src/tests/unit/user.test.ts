...
describe('User Profile & Search API', () => {
    ...
    beforeAll(async () => {
        ...
        // 🚨 Seed User 1: The "Target" user that we want to find in the search
        ...
        // 🚨 Seed User 2: The "Searcher" user who is executing the queries
        ...
        // Capture the cookie from User 2 (Jane)
    ...
    it('should successfully find users using the searchUsers query', async () => {
        // Jane (the logged in user) is searching for John
        const response = await request(app)
            .post('/graphql')
