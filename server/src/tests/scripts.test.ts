...
describe('Script Core API (Create, Read, Update, Delete)', () => {
    ...
    beforeAll(async () => {
        ...
        // 🚨 Create the Author user who will own the scripts
    ...
    it('should allow an authenticated user to create a script', async () => {
        const response = await request(app)
            .post('/graphql')
            .set('Cookie', authorCookie) // 🚨 Attach the cookie
            .send({
                query: `
                    mutation {
                        createScript(
        ...
        expect(script.author.name).toBe("Script Author"); // Proves context.user was linked correctly!
    ...
    it('should successfully delete the script', async () => {
        ...
        // Depending on your resolver, this might return null or throw an error
